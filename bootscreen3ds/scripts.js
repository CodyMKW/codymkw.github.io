/* global $ */
$.jCanvas.defaults.fromCenter = false;

/* .getCanvasImage() don't work on Google Chrome if the page is served from a file URL (file://).
This is a limitation of Google Chrome’s sandboxing architecture, and therefore cannot be fixed */
if (window.location.protocol == 'file:' && window.navigator.vendor == "Google Inc.") {
    $('#offline_warning').show();
    $('select[name=type] option[value=menuhax2015]', "#settings").prop('disabled', true);
    $('select[name=type] option[value=menuhax2016]', "#settings").prop('disabled', true);
}

/* jCanvas has an option for write full strings but don't have a option for control letter spacing.
The font has a letter spacing of 2px, and the generator needs a spacing of 1px.
This function allows to write character by character with only 1px of spacing. */
var write = function (x, y, text, color = 'gray') {
    while (text != '') {
        var letter = text.substr(0, 1);

        /* Search for specials characters */
        if (letter == '_') {
            text = text.substr(1);
            letter = text.substr(0, 1);
            if (color == 'gray') color = 'white';
            else if (color == 'white') color = 'gray';
        }

        /* Draw 1 character */
        $('#topscreen').drawText({
            fillStyle: color,
            x: x + 2,
            y: y,
            fontSize: curDimensions.lh,
            fontFamily: 'PerfectDOSVGA437Win',
            align: 'left',
            text: letter
        });

        /* Remove the written character from the string, and if itn't empty, continue recursive */
        text = text.substr(1);
        x = x + curDimensions.lh / 2;
    }
}

var curDimensions = {
    /*Screen width*/
    w: 400,
    /*Screen heigth*/
    h: 240,
    /*Device (switch or 3ds)*/
    d: '3ds',
    /*line-height*/
    lh: 16,
    /*left padding of top 2 lines*/
    tp: 24
};
var resizeImage = function (device) {
    curDimensions.d = device;
    var imgSrc;
    var imgW;
    var imgH;
    var imgSX;
    var imgSY;
    switch (device) {
        case '3ds':
            curDimensions.w = 400;
            curDimensions.h = 240;
            curDimensions.lh = 16;
            imgSrc = 'images/symbols.png';
            imgW = 21;
            imgH = 29;
            imgSX = 40;
            imgSY = 10;
            curDimensions.tp = 24;
            $('#topscreen').width(curDimensions.w);
            $('#topscreen').height(curDimensions.h);
            break;
        case 'switch':
            curDimensions.w = 1280;
            curDimensions.h = 720;
            curDimensions.lh = 32;
            imgSrc = 'images/symbolsNX.png';
            imgW = 43;
            imgH = 58;
            imgSX = 0;
            imgSY = 336;
            curDimensions.tp = 48;
            $('#topscreen').width(curDimensions.w / 2);
            $('#topscreen').height(curDimensions.h / 2);
            break;
        case 'wiiu':
            curDimensions.w = 1280;
            curDimensions.h = 720;
            curDimensions.lh = 32;
            imgSrc = 'images/symbolsWiiU.png';
            imgW = 43;
            imgH = 58;
            imgSX = 0;
            imgSY = 336;
            curDimensions.tp = 48;
            $('#topscreen').width(curDimensions.w / 2);
            $('#topscreen').height(curDimensions.h / 2);
            break;
    }

    $('#topscreen').attr('width', curDimensions.w);
    $('#topscreen').attr('height', curDimensions.h);

    $('#topscreen').clearCanvas().drawRect({
        fillStyle: 'black',
        x: 0,
        y: 0,
        width: curDimensions.w,
        height: curDimensions.h
    }).drawImage({
        source: imgSrc,
        x: 1,
        y: curDimensions.lh,
        sWidth: imgW,
        sHeight: imgH,
        sx: imgSX,
        sy: imgSY
    });

    //console.log($('#topscreen'));
};

function redraw() {
    var $topscreen = $('#topscreen');

    var model = $('input[name=model]:checked', "#settings").val();
    var region = $('select[name=region] option:selected', "#settings").val();
    var sd = $('select[name=sd] option:selected', "#settings").val();
    var type = $('select[name=type] option:selected', "#settings").val();

    var line1 = $('select[name=type] option:selected', "#settings").text();
    var line2 = '';
    var processor = 0;
    var use_bootinput = false;
    var use_auxinput = false;
    var use_thirdinput = false;
    var use_fourthinput = false;

    if (model == 'wiiU') {
    $('#wiiuOptions').show();
    } else {
    $('#wiiuOptions').hide();
    }

if ($('select[name=boottool] option:selected', "#settings").val() == 'custom') {
    $('input[name=boottool]', "#settings").show();
    $('select[name=boottool]', "#settings").parent().hide();
    use_bootinput = true;
}

if ($('select[name=secondTool] option:selected', "#settings").val() == 'custom') {
    $('input[name=secondTool]', "#settings").show();
    $('select[name=secondTool]', "#settings").parent().hide();
    use_auxinput = true;
}

if ($('select[name=thirdTool] option:selected', "#settings").val() == 'custom') {
    $('input[name=thirdTool]', "#settings").show();
    $('select[name=thirdTool]', "#settings").parent().hide();
    use_thirdinput = true;
}

if ($('select[name=fourthTool] option:selected', "#settings").val() == 'custom') {
    $('input[name=fourthTool]', "#settings").show();
    $('select[name=fourthTool]', "#settings").parent().hide();
    use_fourthinput = true;
}

    switch (model) {
        case '3DS':
            resizeImage('3ds');
            write(0, curDimensions.lh * 5, 'Nintendo 3DS CTR-001(' + region + ')');
            processor = 2;
            sd += ' SD'
            break;
        case '3DSXL':
            resizeImage('3ds');
            if (region == 'JPN')
                write(0, curDimensions.lh * 5, 'Nintendo 3DS LL SPR-001(' + region + ')');
            else
                write(0, curDimensions.lh * 5, 'Nintendo 3DS XL SPR-001(' + region + ')');
            processor = 2;
            sd += ' SD'
            break;
        case '2DS':
            resizeImage('3ds');
            write(0, curDimensions.lh * 5, 'Nintendo 2DS FTR-001(' + region + ')');
            processor = 2;
            sd += ' SD'
            break;
        case 'n2DSXL':
            resizeImage('3ds');
            if (region == 'JPN')
                write(0, curDimensions.lh * 5, 'New Nintendo 2DS LL JAN-001(' + region + ')');
            else
                write(0, curDimensions.lh * 5, 'New Nintendo 2DS XL JAN-001(' + region + ')');
            processor = 4;
            sd += ' microSD'
            break;
        case 'n3DS':
            resizeImage('3ds');
            write(0, curDimensions.lh * 5, 'New Nintendo 3DS KTR-001(' + region + ')');
            processor = 4;
            sd += ' microSD'
            break;
        case 'n3DSXL':
            resizeImage('3ds');
            if (region == 'JPN')
                write(0, curDimensions.lh * 5, 'New Nintendo 3DS LL RED-001(' + region + ')');
            else
                write(0, curDimensions.lh * 5, 'New Nintendo 3DS XL RED-001(' + region + ')');
            processor = 4;
            sd += ' microSD'
            break;
        case 'neonSwitch':
            resizeImage('switch');
            write(0, curDimensions.lh * 5, 'Nintendo Switch HAC-001(' + region + ')');
            processor = 64;
            sd += ' microSD'
            break;
        case 'wiiU':
            resizeImage('wiiu');
            write(0, curDimensions.lh * 5, 'Nintendo Wii U WUP-101(' + region + ')');
            processor = 6;
            sd += ' USB'
            break;
    }
    switch (type) {
        case 'luma2016':
            line2 = 'Copyright(C) 2016, AuroraWright';
            break;
        case 'luma2017':
            line2 = 'Copyright(C) 2017, AuroraWright';
            break;
        case 'luma2018':
            line2 = 'Copyright(C) 2018, AuroraWright';
            break;
        case 'menuhax2015':
            line2 = 'Copyright(C) 2015, yellow8';
            break;
        case 'menuhax2016':
            line2 = 'Copyright(C) 2016, yellow8';
            break;
        case 'reiNX2018':
            write(0, curDimensions.lh * 12, 'Booting Horizon OS...');
            line2 = 'Copyright(C) 2018, Reisyukaku';
            break;
        case 'atmosphereNX2018':
            write(0, curDimensions.lh * 12, 'Booting Horizon OS...');
            line2 = 'Copyright(C) 2019, SciresM, TuxSH, hexkyz, and fincs';
            break;
        case 'lakkaNX2018':
            write(0, curDimensions.lh * 12, 'Booting GNU/Linux...');
            line2 = 'Copyright(C) 2018, Natinusala and libretro';
            break;
        case 'sxOSNX2018':
            write(0, curDimensions.lh * 12, 'Booting Horizon OS...');
            line2 = 'Copyright(C) 2018, Team Xecuter';
            break;
        case 'aroma2022':
            write(0, curDimensions.lh * 12, 'Booting Cafe OS...');
            line2 = 'Copyright(C) 2022, Maschell';
            break;
        case 'aroma2023':
            write(0, curDimensions.lh * 12, 'Booting Cafe OS...');
            line2 = 'Copyright(C) 2023, Maschell';
            break;
        case 'aroma2024':
            write(0, curDimensions.lh * 12, 'Booting Cafe OS...');
            line2 = 'Copyright(C) 2024, Maschell';
            break;
        case 'aroma2025':
            write(0, curDimensions.lh * 12, 'Booting Cafe OS...');
            line2 = 'Copyright(C) 2025, Maschell';
            break;
        case 'aroma2026':
            write(0, curDimensions.lh * 12, 'Booting Cafe OS...');
            line2 = 'Copyright(C) 2026, Maschell';
            break;
    }

    switch ($('select[name=logoOptions] option:selected', "#settings").val()) {
        case 'energyStar':
            $topscreen.drawImage({
                source: 'images/symbols.png',
                x: curDimensions.w - 134,
                y: 16,
                sWidth: 133,
                sHeight: 84,
                sx: 0,
                sy: 0
            }).drawRect({
                fillStyle: 'black',
                x: 306,
                y: 26,
                width: 21,
                height: 29
            });
            break;
        case 'energyLuma':
            $topscreen.drawImage({
                source: 'images/symbols.png',
                x: curDimensions.w - 134,
                y: 16,
                sWidth: 133,
                sHeight: 84,
                sx: 0,
                sy: 84
            });
            break;
        case 'lumaIcon':
            $topscreen.drawImage({
                source: 'images/symbols.png',
                x: curDimensions.w - 134,
                y: 8,
                sWidth: 133,
                sHeight: 84,
                sx: 0,
                sy: 84 * 2
            });
            break;
        case 'reiNxLogo':
            $topscreen.drawImage({
                source: 'images/symbolsNX.png',
                x: curDimensions.w - 134,
                y: 8,
                sWidth: 133,
                sHeight: 84,
                sx: 0,
                sy: 84
            });
            break;
        case 'atmosphereNxLogo':
            $topscreen.drawImage({
                source: 'images/symbolsNX.png',
                x: curDimensions.w - 134,
                y: 8,
                sWidth: 133,
                sHeight: 84,
                sx: 0,
                sy: 0
            });
            break;
        case 'lakkaNxLogo':
            $topscreen.drawImage({
                source: 'images/symbolsNX.png',
                x: curDimensions.w - 134,
                y: 8,
                sWidth: 133,
                sHeight: 84,
                sx: 0,
                sy: 84 * 2
            });
            break;
        case 'xsOsLogo':
            $topscreen.drawImage({
                source: 'images/symbolsNX.png',
                x: curDimensions.w - 134,
                y: 8,
                sWidth: 133,
                sHeight: 84,
                sx: 0,
                sy: 84 * 3
            });
            break;
        case 'aromaLogo':
            $topscreen.drawImage({
                source: 'images/symbolsWiiU.png',
                x: curDimensions.w - 134,
                y: 8,
                sWidth: 133,
                sHeight: 84,
                sx: 0,
                sy: 0
            });
            break;
    }

    write(curDimensions.tp, curDimensions.lh * 1, line1);
    write(curDimensions.tp, curDimensions.lh * 2, line2);

    switch (processor) {
        case 2:
            write(0, curDimensions.lh * 7, 'Main Processor       : Dual-core ARM11 MPCore');
            write(0, curDimensions.lh * 8, 'Memory Testing       : 131072K OK');
            break;
        case 4:
            write(0, curDimensions.lh * 7, 'Main Processor       : Quad-core ARM11 MPCore');
            write(0, curDimensions.lh * 8, 'Memory Testing       : 262144K OK');
            break;
        case 64:
            write(0, curDimensions.lh * 7, 'Main Processor       : NVIDIA Tegra X1');
            write(0, curDimensions.lh * 8, 'Memory Testing       : 4194304K OK');
            break;
        case 6:
            write(0, curDimensions.lh * 7, 'Main Processor       : Tri-core IBM PowerPC (Espresso)');
            write(0, curDimensions.lh * 8, 'Memory Testing       : 2097152K OK');
            break;
    }

    var nandSize = document.getElementById("nandSize").value;
    if (nandSize && nandSize > 0) {
        write(0, curDimensions.lh * 9, 'Detecting Primary Master ... ' + nandSize + 'G Internal Memory');
    } else {
        write(0, curDimensions.lh * 9, 'Detecting Primary Master ... ' + processor / 2 + 'G Internal Memory');
    }


    var slaveLabel = (curDimensions.d == 'wiiu') ? 'Drive' : 'Card';
    write(0, curDimensions.lh * 10, 'Detecting Primary Slave  ... ' + sd + ' ' + slaveLabel);

if (!use_bootinput)
    $('input[name=boottool]', "#settings").val($('select[name=boottool] option:selected', "#settings").text());

if (!use_auxinput)
    $('input[name=secondTool]', "#settings").val($('select[name=secondTool] option:selected', "#settings").text());

if (!use_thirdinput)
    $('input[name=thirdTool]', "#settings").val($('select[name=thirdTool] option:selected', "#settings").text());

if (!use_fourthinput)
    $('input[name=fourthTool]', "#settings").val($('select[name=fourthTool] option:selected', "#settings").text());

    var boot_bool = $('input[name=hold]', "#settings").is(':checked');
    var boot_keys = $('select[name=onboot] option:selected', "#settings").val();
    var boot_tool = $('input[name=boottool]', "#settings").val();
    var boot_text = '_Hold ' + boot_keys + ' ' + $('select[name=firstTime] option:selected').text() + '_ to enter _' + boot_tool + '_.';

    var aux_bool = $('input[name=secondLine]', "#settings").is(':checked');
    var aux_keys = $('select[name=secondButton] option:selected').val();
    var aux_tool = $('input[name=secondTool]').val();
    var aux_text = '_Hold ' + aux_keys + ' ' + $('select[name=secondTime] option:selected').text() + '_ to enter _' + aux_tool + '_.';

var third_bool = $('input[name=thirdLine]', "#settings").is(':checked');
var third_keys = $('select[name=thirdButton] option:selected').val();
var third_tool = $('input[name=thirdTool]').val();
var third_text = '_Hold ' + third_keys + ' ' + $('select[name=thirdTime] option:selected').text() + '_ to enter _' + third_tool + '_.';

var fourth_bool = $('input[name=fourthLine]', "#settings").is(':checked');
var fourth_keys = $('select[name=fourthButton] option:selected').val();
var fourth_tool = $('input[name=fourthTool]').val();
var fourth_text = '_Hold ' + fourth_keys + ' ' + $('select[name=fourthTime] option:selected').text() + '_ to enter _' + fourth_tool + '_.';

var bottomText1Y = 13;
var bottomText2Y = 14;
if (curDimensions.d == 'switch') {
    var bottomText1Y = 20.5;
    var bottomText2Y = 21.5;
}
if (curDimensions.d == 'wiiu') {
    var bottomText1Y = 19.5;
    var bottomText2Y = 20.25;
}

if (boot_bool && !aux_bool)
    write(0, curDimensions.lh * bottomText2Y, boot_text);
else if (boot_bool)
    write(0, curDimensions.lh * bottomText1Y, boot_text);

if (aux_bool)
    write(0, curDimensions.lh * bottomText2Y, aux_text);

if (third_bool && curDimensions.d == 'wiiu')
    write(0, curDimensions.lh * (bottomText2Y + 0.65), third_text);

if (fourth_bool && curDimensions.d == 'wiiu')
    write(0, curDimensions.lh * (bottomText2Y + 1.3), fourth_text);

    if ($topscreen.width() == curDimensions.w * 2) {
        $topscreen.drawImage({
            source: $topscreen.getCanvasImage(),
            x: curDimensions.w,
            y: 0
        });
    }

}

/* This draw the entire splash screen with any change on the form */
$("#settings input, #settings select").on('change', function () {
    redraw();
});

window.onload = function () {

    $('canvas').drawImage({
        source: 'images/symbols.png',
        x: 0,
        y: 0,
        load: function () {
            $("select[name=region]", "#settings").trigger('change');
            if ($('#offline_warning').is(':hidden'))
                $('#downloadPNG, #downloadBIN').removeClass('disabled');
        }
    });

};

$('input[name=boottool]', "#settings").keyup(function () {
    $("#settings input").trigger('change');
});
$('input[name=auxtool]', "#settings").keyup(function () {
    $("#settings input").trigger('change');
});
$('input[name=thirdTool]', "#settings").keyup(function () {
    $("#settings input").trigger('change');
});
$('input[name=fourthTool]', "#settings").keyup(function () {
    $("#settings input").trigger('change');
});


/* Create a PNG downloadable of the canvas */
/* global download */
$('#downloadPNG').click(function () {
    if (!$(this).hasClass('disabled')) {
        var filename = ($('#topscreen').width() == curDimensions.w) ? 'splash.png' : 'imagedisplay.png';
        var filedata = $('#topscreen').getCanvasImage();
        download(filedata, filename, "image/png");
    }
});

$('#downloadBIN').click(function () {
    if (!$(this).hasClass('disabled')) {
        var filename = ($('#topscreen').width() == curDimensions.w) ? 'splash.bin' : 'menuhax_imagedisplay.bin';

        var width = $('#topscreen').height();
        var height = $('#topscreen').width();

        var $canvas = $('<canvas/>').css({
            position: 'absolute',
            top: 0,
            left: -1 * width
        }).appendTo('body');
        $canvas.attr('width', width).attr('height', height);

        $canvas.drawImage({
            source: $('#topscreen').getCanvasImage(),
            x: width / 2,
            y: height / 2,
            fromCenter: true,
            rotate: 90
        });

        var canvasdata = $canvas.get(0).getContext('2d').getImageData(0, 0, width, height).data;
        var filedata = '';

        for (var i = 0; i < canvasdata.length; i += 4)
            filedata += String.fromCharCode(canvasdata[i + 2], canvasdata[i + 1], canvasdata[i]);

        $canvas.remove();
        download('data:application/octet-stream;base64,' + window.btoa(filedata), filename);
    }
});
