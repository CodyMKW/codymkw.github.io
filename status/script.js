...
<script>
    ...
    // Update the copyright year range dynamically
    function updateCopyrightYearRange() {
        const startYear = 2023; // Change this to the year you started
        const currentYear = new Date().getFullYear();
        const yearRange = startYear === currentYear ? startYear : `${startYear} - ${currentYear}`;
        document.getElementById('copyrightYear').textContent = yearRange;
    }

    // Call the function to update the copyright year range
