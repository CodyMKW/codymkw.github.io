...
function applyFiltersAndSearch() {
    ...
    filteredPosts = posts.filter(function(post) {
        ...
        var matchesSearch = !query || (post.title && post.title.toLowerCase().indexOf(query) !== -1) || (post.content && post.content.toLowerCase().indexOf(query) !== -1) || (post.content2 && post.content2.toLowerCase().indexOf(query) !== -1);
...
function renderListView() {
    ...
    pagePosts.forEach(function(post) {
        ...
        if (post.content) {
            ...
            previewHTML = '<div class="post-content">' + marked.parse(trimmedContent) + '</div>' + '<a href="?post=' + post.originalIndex + '" class="read-more">Read more →</a>';
        } else if (post.video || post.content2) {
            previewHTML = (post.video ? '<iframe src="' + post.video + '" frameborder="0" allowfullscreen></iframe>' : "") + (post.content2 ? '<div class="post-content">' + marked.parse(post.content2) + '</div>' : "");
        }
        postElement.innerHTML = '<h3><a href="?post=' + post.originalIndex + '">' + post.title + '</a></h3>' + '<p class="post-meta" id="blogmetadata">' + post.date + ' • ' + post.time + ' • ' + post.author + ' • ' + post.category + '</p>' + (post.image ? '<img src="' + post.image + '" alt="Post Image">' : "") + previewHTML;
...
function renderSinglePostView(postId) {
    ...
    var postHTML = '<a href="#" class="blog-back-button">‹ All Posts</a>' + '<div class="blog-post" data-index="' + post.originalIndex + '">' + '<h3>' + post.title + '</h3>' + '<p class="post-meta" id="blogmetadata">' + post.date + ' • ' + post.time + ' • ' + post.author + ' • ' + post.category + '</p>' + (post.image ? '<img src="' + post.image + '" alt="Post Image">' : "") + '<div class="post-content">' + (post.content ? marked.parse(post.content) : "") + '</div>' + (post.video ? '<iframe src="' + post.video + '" frameborder="0" allowfullscreen></iframe>' : "") + '<div class="post-content">' + (post.content2 ? marked.parse(post.content2) : "") + '</div>' + '<hr style="margin: 2em 0; border: none; border-top: 1px solid #ccc;">' + '<div id="disqus_thread" style="margin-top: 2em;"></div>' + '</div>';
