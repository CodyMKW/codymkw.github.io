...
function handleRouting() {
    ...
    if (postId !== null) {
        ...
        var searchBar = document.getElementById("searchBar");
        var categoryFilter = document.getElementById("categoryFilter");
        if (searchBar) searchBar.value = urlParams.get("search") || "";
        if (categoryFilter) categoryFilter.value = urlParams.get("category") || "all";
...
function updateURLFromFilters() {
    ...
    var searchBar = document.getElementById("searchBar");
    var categoryFilter = document.getElementById("categoryFilter");
    var query = searchBar ? searchBar.value.trim() : "";
...
function renderRatingBlock(ratingValue) {
    if (!ratingValue) return '';
    var num = parseFloat(ratingValue);
    var displayContent = '';
    ...
    if (!isNaN(num) && num <= 5) {
        ...
        for (var j = 0; j < emptyStars; j++) stars += '☆';

        var verdict = '';
        if (num >= 5) verdict = 'Masterpiece';
        else if (num >= 4.5) verdict = 'Outstanding';
        else if (num >= 4) verdict = 'Excellent';
        else if (num >= 3.5) verdict = 'Great';
        else if (num >= 3) verdict = 'Good';
        else if (num >= 2.5) verdict = 'Average';
        else if (num >= 2) verdict = 'Below Average';
        else if (num >= 1) verdict = 'Poor';
        else verdict = 'Avoid';

        displayContent = '<div style="font-size: 2em; color: #FFD700; text-shadow: 1px 1px 2px rgba(0,0,0,0.1);">' + stars + '</div>' +
            '<div style="font-size: 0.9em; color: #666; margin-top: 5px;">' + ratingValue + ' / 5</div>' +
            '<div style="font-size: 1em; font-weight: bold; margin-top: 6px;">' + verdict + '</div>';
    } else {
        displayContent = '<div style="font-size: 1.5em; font-weight: bold; color: #4CAF50;">' + ratingValue + '</div>';
    }
    return '<div class="blog-review-rating">' +
        '<h4>Review Score</h4>' +
        displayContent +
        '</div>';
}
...
function renderSinglePostView(postId) {
    ...
    var postHTML =
        ...
        (post.rating ? renderRatingBlock(post.rating) : "") +
