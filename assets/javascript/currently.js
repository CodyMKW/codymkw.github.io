function fetchLastWatchedAnime(username) {
    ...
    return fetch("https://graphql.anilist.co", {
        ...
        return {
            animeName: "Nothing at the moment",
            span: null
        };
    }).catch(function() {
        return {
            animeName: "Error loading",
            span: null
        };
    });
}
...
function applyEpisodeColor() {
    ...
    style.textContent = ".anime-episode {" + "color:" + color + ";" + "font-weight:600;margin-left:6px;display:inline-block;" + "text-shadow:0 0 6px " + (theme === "dark" ? "rgba(0,201,0,0.6)" : "rgba(59,130,246,0.5)") + ";" + "transition:all 0.3s ease-in-out;}" + ".anime-episode:hover {" + "color:" + (theme === "dark" ? "#33ff33" : "#60A5FA") + ";" + "text-shadow:0 0 10px " + (theme === "dark" ? "rgba(0,255,0,0.8)" : "rgba(96,165,250,0.9)") + ";}" + "@media (max-width:600px){.anime-episode{font-size:0.95em;display:inline;}}";
}
...
function fetchStatusCafe() {
    ...
    return fetch("https://status.cafe/users/" + username + "/status.json?t=" + Date.now()).then(function(res) {
...
function fetchLastFmTrack() {
    ...
    var url = "https://ws.audioscrobbler.com/2.0/?method=user.getrecenttracks&user=" + username + "&api_key=" + apiKey + "&limit=1&format=json&t=" + Date.now();
...
function loadCurrently() {
    return fetch("https://nxapi-presence.fancy.org.uk/api/presence/644cd5195d154bd5?t=" + Date.now()).then(function(presenceRes) {
        ...
        if (cafeBox) {
            cafeBox.innerHTML = '<div class="statuscafe-entry"><a href="https://status.cafe/users/' + data.statusCafe.author + '" target="_blank"><strong>' + data.statusCafe.author + '</strong></a> ' + data.statusCafe.face + '<span style="opacity:0.7; margin-left:6px;">' + data.statusCafe.time + '</span><p>' + data.statusCafe.content + '</p></div>';
        }
        var container = document.getElementById("currently-section");
        if (container) {
            container.innerHTML = '<ul class="currently-list">' + '<li><strong>🎮 Playing:</strong><br> ' + data.playing + '</li>' + '<li><strong>🎵 Listening:</strong><br>' + (data.lastFm.link ? '<a href="' + data.lastFm.link + '" target="_blank">' + data.lastFm.song + '</a>' : data.lastFm.song) + '<span style="opacity:0.7; margin-left:6px;">' + data.lastFm.ago + '</span></li>' + '<li><strong>📺 Watching:</strong><br></li></ul>';
...
function loadLatestPosts() {
    return fetch("https://api.npoint.io/5ac2ef5dd46fbff62a02").then(function(response) {
